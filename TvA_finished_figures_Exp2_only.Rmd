...
#### TvA analysis script - manuscripts

This script produces figures. Data for plotting is taken from the Data folder and the relevant sub folders within them. 
...
"generate timecourse dataframe"
rmarkdown::render(here::here("TvA_analysis/Generating_Timecourse_Data.Rmd"))

"coordinates for plots"
trajectory <- read.csv(here::here("TvA_analysis/Data/Exp_Design_Data/trajectory.csv"))
experiment_2_coordinates <- read.csv(here::here("TvA_analysis/Data/Exp_Design_Data/experiment_2_coordinates.csv"))
```
...
## Figure 1 Theoretical differences between Threshold and Accumulator framework
...
ggplot() +
  ...
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig1.tiff"), plot = last_plot(), width = 7, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 3 Bird's eye view of experimental paradigm. 
...
ggplot() +
  ...
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig3.tiff"), plot = last_plot(), width = 12, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 4 Schematic representation of differences between Threshold and Accumulator predictions. 

```{r}
thresh.1 <- read.csv(here::here("TvA_analysis/Data/Sim_Data/thresh_1.csv"), header = FALSE) %>%
  dplyr::mutate(X = row_number()) %>%
  dplyr::mutate(startingpos = 0)

thresh.2 <- read.csv(here::here("TvA_analysis/Data/Sim_Data/thresh_2.csv"), header = FALSE) %>%
  dplyr::mutate(X = row_number()) %>%
  dplyr::mutate(startingpos = 4)
  
thresh.3 <- read.csv(here::here("TvA_analysis/Data/Sim_Data/thresh_3.csv"), header = FALSE) %>%
  dplyr::mutate(X = row_number()) %>%
...
ggplot() +
  ...
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig4.tiff"), plot = last_plot(), width = 7, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 5: Accumulator vs Threshold predictions + RT data.

```{r}
"Loading in Threshold and Accumulator predictions"
df_accum <- read.csv(here::here("TvA_analysis/Data/Sim_Data/df_accum.csv")) %>%
  dplyr::mutate(model = "accumulator", heading = as.numeric(heading))

df_thresh <- read.csv(here::here("TvA_analysis/Data/Sim_Data/df_thresh.csv")) %>%
  dplyr::mutate(model = "threshold", heading = as.numeric(heading))

"combing dataframes"
sim_preds_response <- rbind(df_accum, df_thresh)

"RT predictions and data"
acc.plot <- ggplot(sim_preds_response %>%
         dplyr::filter(model == "accumulator"), mapping = aes(x = heading, y = timestamp, linetype = as.factor(startingpos))) +
  geom_line() +
...
thresh.plot <- ggplot(sim_preds_response %>%
         dplyr::filter(model == "threshold"), mapping = aes(x = heading, y = timestamp, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_linetype_manual(name = "Starting position (m)", labels = c("0", "4", "8"),  values = c("dotted", "dashed", "solid")) +
  scale_x_continuous(name = "Orientation (°)", breaks = c(1, 2)) +
  ylab("Reaction time (s)") +
...
g <- arrangeGrob(acc.plot, thresh.plot, data.plot, nrow = 1)
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig5.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 6: Accumulator vs Threshold predictions + lateral position error data.

```{r}
"LPE predictions and data"
acc.plot <- ggplot(sim_preds_response %>%
         dplyr::filter(model == "accumulator"), mapping = aes(x = heading, y = latpos, linetype = as.factor(startingpos))) +
  geom_line() +
...
thresh.plot <- ggplot(sim_preds_response %>%
         dplyr::filter(model == "threshold"), mapping = aes(x = heading, y = latpos, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_linetype_manual(name = "Starting position (m)", labels = c("0", "4", "8"),  values = c("dotted", "dashed", "solid"), guide = guide_legend(reverse = TRUE)) +
  scale_x_continuous(name = "Orientation (°)", breaks = c(1, 2)) +
  ylab("Lateral position error (m)") +
...
g <- arrangeGrob(acc.plot, thresh.plot, data.plot, nrow = 1)
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig6.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 7: Accumulator vs Threshold predictions + steering rate error data.

```{r}
sim_preds_error_swa <- sim_preds_response %>%
  dplyr::mutate(SWA = latpos) %>%
...
acc.plot <- ggplot(sim_preds_error_swa %>%
         dplyr::filter(model == "accumulator"), mapping = aes(x = heading, y = SWA, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_linetype_manual(name = "Starting position (m)", labels = c("0", "4", "8"),  values = c("dotted", "dashed", "solid"), guide = guide_legend(reverse = TRUE)) +
  scale_x_continuous(name = "Orientation (°)", breaks = c(1, 2)) +
...
thresh.plot <- ggplot(sim_preds_error_swa %>%
         dplyr::filter(model == "threshold"), mapping = aes(x = heading, y = SWA, linetype = as.factor(startingpos))) +
  geom_line() +
  scale_linetype_manual(name = "Starting position (m)", labels = c("0", "4", "8"),  values = c("dotted", "dashed", "solid"), guide = guide_legend(reverse = TRUE)) +
  scale_x_continuous(name = "Orientation (°)", breaks = c(1, 2)) +
  ylab("Steering wheel angle (°)") +
...
g <- arrangeGrob(acc.plot, thresh.plot, data.plot, nrow = 1)
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig7.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 8: comparison of steering wheel angle over time and steering rate over time. 
...
plot.steering.dat <- true_responses %>%
  ...
  dplyr::filter(ppid_trialn == "14_3_10")

swa.plot <- ggplot() +
  geom_line(plot.steering.dat, mapping = aes(x = timestamp, y = SWA)) +
  geom_point(magnitudedata %>%
               dplyr::filter(ppid_trialn == "14_3_10"), mapping = aes(x = PeakSteeringTime, y = peakSWA)) +
  scale_x_continuous(limits = c(0, 2.6), expand = c(0, 0)) +
  scale_y_continuous(limits = c(-8, 15), expand = c(0, 0)) +
  ggtitle("B") +
  ylab("Steering wheel angle (°)") +
  xlab("Time (s)") +
  theme_plot +
...
ggsave(here::here("TvA_analysis/Manuscript plots/Fig8.tiff"), g,  width = 15, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```

## Figure 9: birdseye trajectory plot and average position relative to road-line at response. 
...
ggplot() +
  ...
  ggsave(here::here("TvA_analysis/Manuscript plots/Fig9.tiff"), plot = last_plot(), width = 10, height = 7, units = 'cm', dpi = 600, type = 'cairo')
```
...
stargazer(fit.gamma.swa, fit.gamma.swa, fit.gamma.swa, type = "text", 
          ...
          out = here::here("TvA_analysis/Manuscript plots/random.effects.exp2.html")
          )
```
